 ...
 */
public class TakeOfferDataModel extends OfferDataModel {
    ...
    protected void activate() {
        ...
        tradeManager.checkOfferAvailability(offer,() -> {}, errorMessage -> {
  ...
  /*  private void setFeeFromFundingTx(Coin fee) {
    ...
    public void preflight(Offer offer, PaymentAccount account, CompletableFuture<Message> promise) {
        message.success = false;
        if(!hasAcceptedArbitrators()){
    ...
    public void commit(Offer offer, PaymentAccount account,Coin amount, CompletableFuture<Message> promise){
        ...
        if(isAmountLargerThanOfferAmount()){
    ...
    public void getTxFee(Offer offer,CompletableFuture<Coin> promise){
        initWithData(offer);
        promise.complete(getTotalTxFee());
        deactivate();
        onClose();
    }
}
