 ...
 */
public class CreateOfferDataModel extends OfferDataModel {
    ...
    public void preflight(
        ...
        String priceCode = account.getSelectedTradeCurrency().getCode();
        setUseMarketBasedPrice(priceModel.equals("PERCENTAGE"));
        setAmount(amount);
        ...
        if(priceModel.equals("PERCENTAGE")){
            ...
            setMarketPriceAvailable(true);
            setMarketPriceMargin(margin);
        ...
        if(p2PService.getAddress() == null){
            message.success = false;
            message.message = "Address must not be null";
            promise.complete(message);
            return;
        }
        if(getMakerFee() == null){
            message.success = false;
            message.message = "makerFee must not be null";
            promise.complete(message);
            return;
        }
        if(buyerSecurityDepositAsCoin.compareTo(Restrictions.getMaxBuyerSecurityDeposit()) >= 0){
            message.success = false;
            message.message = "securityDeposit must be not exceed "+Restrictions.getMaxBuyerSecurityDeposit().toFriendlyString();
            promise.complete(message);
            return;
        }
        if(buyerSecurityDepositAsCoin.compareTo(Restrictions.getMinBuyerSecurityDeposit()) <= 0){
            message.success = false;
            message.message = "securityDeposit must be not be less than "+Restrictions.getMinBuyerSecurityDeposit().toFriendlyString();
            promise.complete(message);
            return;
        }
        if(filterManager.isCurrencyBanned(priceCode)){
            message.success = false;
            message.message = Res.get("offerbook.warning.currencyBanned");
            promise.complete(message);
            return;
        }
        if(filterManager.isPaymentMethodBanned(paymentAccount.getPaymentMethod())){
            message.success = false;
