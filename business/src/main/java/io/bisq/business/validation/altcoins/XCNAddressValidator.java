...
package io.bisq.business.validation.altcoins;
...
public class XCNAddressValidator
{
	...
	{
		if (addr.length() != 34)
			return new InputValidator.ValidationResult(false, "XCN_Addr_Invalid: Length must be 34!");
		if (!addr.startsWith("C"))
			return new InputValidator.ValidationResult(false, "XCN_Addr_Invalid: must start with 'C'!");
		byte[] decoded = decodeBase58(addr, 58, 25);
		if (decoded == null)
			return new InputValidator.ValidationResult(false, "XCN_Addr_Invalid: Base58 decoder error!");
		...
		if(hash == null || !Arrays.equals(Arrays.copyOfRange(hash, 0, 4), Arrays.copyOfRange(decoded, 21, 25)))
			return new InputValidator.ValidationResult(false, "XCN_Addr_Invalid: Checksum error!");
